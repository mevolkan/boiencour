...
if (isset($_GET['delete_id'])) {
  $sql_query = "DELETE FROM music WHERE id=" . $_GET['delete_id'];
  mysqli_query($con, $sql_query);
  header("Location: $_SERVER[PHP_SELF]");
  }
if (isset($_GET['changestatus_id'])) {
  $sql_query = "UPDATE music SET `status`='" . $_GET['status'] . "' WHERE id=" . $_GET['changestatus_id'];
  mysqli_query($con, $sql_query);
  header("Location: $_SERVER[PHP_SELF]");
  }
?>
...
<head>
  ...
  <script type="text/javascript">
    function edt_id(id) {
      window.location.href = 'edit_music.php?edit_id=' + id;
    }
    function view_id(id) {
      window.location.href = 'view_music.php?view_id=' + id;
    }
    function delete_id(id) {
      if (confirm('Sure to Delete ?')) {
        window.location.href = 'indexmusic.php?delete_id=' + id;
      }
    }
    function changestatus_id(id, status) {
      window.location.href = 'indexmusic.php?changestatus_id=' + id + '&status=' + status;
    }
  </script>
</head>
  ...
  <div class="container">
    <div class="table-responsive">
      ...
      <table class="table table-striped">
        <tr>
          <th colspan="5"><a href="add_music.php">add music.</a></th>
        </tr>
        <th>SL NO</th>
        <th>name</th>
